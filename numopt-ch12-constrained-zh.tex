...
\begin{frame}{先认三个记号（看不懂公式只看这页就够）}
  \begin{itemize}
    \item $\grad f$（梯度）：指向“最快变好”方向的箭头；这就是训练里做梯度下降时用的那个梯度。
    \item $\grad f=0$：四面八方都平了，往哪边小走一步都没有一阶改善。
    \item 法向：垂直戳出约束曲线的方向，像车轮的辐条；在边界上，它告诉你限制从哪边顶住你。
    \item $\lambda$（拉格朗日乘子）：一个“价格”数字，告诉你把限制放松一点点能省多少；这里的 $\lambda$ 不是学习率。
  \end{itemize}
  \TLtakeaway{先把梯度、法向、价格这三件事放进脑子；后面的公式只是把这三句话写精确。}
\end{frame}
...
\begin{frame}{一句话：约束优化在解决什么}
  \begin{columns}[T]
    ...
    \column{0.52\textwidth}
      ...
      {\scriptsize 示意图。}
  \end{columns}
  \TLtakeaway{约束优化不是硬套“无约束时的最优条件”；而是看允许方向里是否还能下降。}
\end{frame}
...
\begin{frame}{为什么 $\grad f=0$ 失效：一张图}
  \begin{columns}[T]
    ...
    \column{0.54\textwidth}
      ...
      {\scriptsize 示意图。}
  \end{columns}
  \TLtakeaway{关键不是“目标的梯度消失”，而是“目标的下降方向与约束边界对齐（成比例）”。}
\end{frame}

\begin{frame}{KKT 一句话 + 乘子是影子价格}
  \begin{itemize}
    \item KKT 把“相切”推广到很多等式和不等式约束。
    \item 在最优点，目标的“推力”可以由真正卡住你的约束共同解释；不起作用的限制不施力。
    \item 还要满足可行性、乘子符号与互补性：没有卡住的限制，对应价格为零。
    \item 乘子的影子价格直觉：把某条限制放松一单位，最优目标值大约能改善多少。
    \item 直觉：乘子越大，放松那条限制越值钱；乘子为零，说明它不是瓶颈。
...
\begin{frame}{在哪儿用 + 怎么读这份讲义}
  ...
  \begin{itemize}
    ...
    \item $\blacktriangleright$ \textbf{证明读法}：第一遍先抓每个证明帧的“要点”；第二遍回来看 Farkas、切锥、向锥投影等证明细节——这些正是你需要的数学，已为你拆成小步。
...
\begin{frame}{一般约束优化问题：先看问题长什么样}
  这是你在 constrained training 里已经见过的形状：最小化 loss，同时满足硬约束。
  \[
    \min_{x\in\R^{n}} f(x)
    \quad\text{s.t.}\quad
    c_i(x)=0\ (i\in\Eset),\qquad
    c_i(x)\ge0\ (i\in\Iset).
  \]
  \begin{itemize}
    \item $x$ 是模型参数向量；$f(x)$ 是要最小化的目标，也就是 loss。
    \item $c_i(x)$ 是第 $i$ 条限制；等式限制必须刚好满足，不等式限制像预算下限或安全余量。
    \item $\Eset$ 收集等式限制的编号，$\Iset$ 收集不等式限制的编号。
  \end{itemize}
  \TLterm{可行集}就是所有守规矩的参数：
  \[
    \feas=\{\,x\mid c_i(x)=0\ (i\in\Eset),\ c_i(x)\ge0\ (i\in\Iset)\,\},
    \qquad \min_{x\in\feas} f(x).
  \]
  \TLtakeaway{先把它读成一个受限训练问题：参数要让 loss 小，但只能在可行集里移动。}
\end{frame}

\begin{frame}{一般约束优化问题：为什么旧条件会坏}
  无约束优化里，局部极小点常用两条条件检查：
  \[
    \grad f(\xs)=0,\qquad \grad^{2} f(\xs)\succeq0.
  \]
  \begin{itemize}
    \item 这两条条件默认你能朝任意小方向试探。
    \item 约束会删掉一部分方向；在边界上，有些“会让目标更好”的方向是禁止的。
    \item 所以梯度不必消失；真正需要检查的是：所有允许方向里是否还存在下降方向。
    \item 后面会把“允许方向”精确化为切锥，再把“无下降方向”精确化为 KKT。
  \end{itemize}
  \TLtakeaway{约束打破的不是微积分，而是“可以朝任意方向走”这个隐含前提。}
\end{frame}
...
\begin{frame}{揭示：拉格朗日条件}
  ...
  \medskip
  在一个等式约束的 $\mathbb{R}^2$ 问题里，切方向构成一条直线：若 $d$ 是切向，则 $-d$ 也是切向。把"没有下降方向"同时用到 $d$ 和 $-d$：$\grad f(\xs)^{\T} d\ge0$ 且 $\grad f(\xs)^{\T}(-d)\ge0$，两者合起来强制 $\grad f(\xs)^{\T} d=0$。所以 $\grad f$ 与每个切向正交，只能沿法向 $\grad c_1$。

  成对的方向 $d$ 与 $-d$ 把一个不等式夹成等式。
\end{frame}

\begin{frame}{揭示：拉格朗日条件（续）}
  \begin{itemize}
...
\begin{frame}{先认两个术语}
  \begin{itemize}
    \item \TLterm{锥 (cone)}：一族方向，对正数缩放封闭；如果 $d$ 是合法方向，那么 $\alpha d$（$\alpha>0$）也合法。
    \item ML 锚点：把“所有合法的梯度步方向”想成一个楔形；步子放大或缩小，方向族仍然合法。
    \item 小 $o$（$o(\cdot)$，little-o）：比步长更快趋于零的“零头”。
    \item 如果步长是 $h$，$o(h)/h\to0$；它就是 Taylor 展开里你丢掉的高阶项。
  \end{itemize}
  \TLtakeaway{锥负责描述“能往哪些方向走”；小零头负责说明“线性近似之外还剩多少误差”。}
\end{frame}
...
\begin{frame}{记号别搞混}
  \TLinfoblock{记号别搞混（尤其写代码的人）}{%
  \begin{itemize}
    \item $\lambda$ 是乘子 / 价格，不是学习率。
    \item $T_{\feas}$ 是\emph{切锥}，不是转置；转置在本讲义写成上标 $\T$（$d\T\grad f$）。
    \item $N$（法锥）不是维数 $n$。
    \item $\Aset$（起作用集）不是某个矩阵 $A$（\S\,6 后面确实另有一个矩阵 $A$，注意区分）。
  \end{itemize}}
\end{frame}
...
\begin{frame}{定义：可行序列与切锥}
  ...
  \]
  \medskip
  \textbf{为什么余项是 $o(\cdot)$：}$(z_k-\xs)/\|z_k-\xs\|\to d$ 恰好是说 $z_k-\xs-\|z_k-\xs\|\,d$ 比 $\|z_k-\xs\|$ 更快趋于零，即 $=o(\|z_k-\xs\|)$。后面每个证明都把这一行除以 $\|z_k-\xs\|$ 再令 $k\to\infty$。
...
\begin{frame}{揭示：定理 12.2 及其证明}
  ...
  \]
  \proofskipnote
  因为 $o(\|z_k-\xs\|)/\|z_k-\xs\|\to0$，当 $k$ 充分大时余项绝对值小于 $\half|d^{\T}\grad f|\,\|z_k-\xs\|$，被半个一阶项压过。
...
\begin{frame}{引理 12.3(ii) 的证明 —— 用隐函数定理造路径}
给定 $d\in F_1$，$\|d\|=1$。记 $A=[\grad c_i(\xs)^{\T}]_{i\in\Aset}$（行 = 起作用约束梯度）。LICQ 就是 $A$ 行满秩。取 $Z$，其列是 $A$ 零空间的一组基（$AZ=0$）。
  \proofskipnote

\TLinfoblock{隐函数定理}{若 $R(z,t)$ 光滑、$R(\xs,0)=0$，且 $\partial R/\partial z$ 在 $(\xs,0)$ 非奇异，则在 $t=0$ 附近存在唯一光滑的 $z(t)$ 使 $R(z(t),t)=0$、$z(0)=\xs$。\\[2pt]\textit{直觉：非奇异雅可比附近方程组可被光滑反解——就像 Newton 迭代在那里必定局部收敛。}}

\medskip
\textbf{为什么雅可比非奇异：}把 $A$（$m\times n$ 行满秩）与 $Z^{\T}$（$(n-m)\times n$）叠成 $\begin{psmallmatrix}A\\ Z^{\T}\end{psmallmatrix}$（$n\times n$）。$A$ 的行空间与 $A$ 的零空间互补、合起来张成 $\mathbb{R}^{n}$，故叠成的方阵可逆。
\end{frame}

\begin{frame}{引理 12.3(ii) 的证明 —— 路径以 $d$ 为极限方向}
令 $R(z,t)=\begin{psmallmatrix} c_{\Aset}(z)-t\,A d\\ Z^{\T}(z-\xs-t d)\end{psmallmatrix}$，则 $R(\xs,0)=0$，且 $\partial R/\partial z|_{(\xs,0)}=\begin{psmallmatrix}A\\ Z^{\T}\end{psmallmatrix}$ 非奇异。隐函数定理给出唯一光滑的 $z_t$，$z_0=\xs$。

\medskip
\textbf{可行性：}由构造 $c_i(z_t)=t\,\grad c_i(\xs)^{\T} d$：在 $\Eset$ 上 $\grad c_i^{\T} d=0$ 故 $=0$；在起作用 $\Iset$ 上 $\grad c_i^{\T} d\ge0$ 故 $\ge0$。不起作用约束在小 $t$ 下因连续性仍 $>0$。所以 $z_t$ 可行。

\medskip
\textbf{极限方向：}对 $z_t$ 在 $t=0$ 求导：$z'(0)$ 满足 $\begin{psmallmatrix}A\\ Z^{\T}\end{psmallmatrix}z'(0)=\begin{psmallmatrix}A d\\ Z^{\T} d\end{psmallmatrix}=\begin{psmallmatrix}A\\ Z^{\T}\end{psmallmatrix}d$，故 $z'(0)=d$。于是 $(z_t-\xs)/t\to d$，从而 $(z_t-\xs)/\|z_t-\xs\|\to d$。$\square$
\end{frame}
...
\begin{frame}{预备知识回顾：Farkas 引理（续）}
  \medskip
  \textbf{二维直觉：}把起作用梯度 $a_i$ 看成从原点射出的箭头，它们张成一个锥。要么 $g$ 落在锥内（可写成非负组合 (I)）；要么落在锥外，于是有一张超平面把 $g$ 与锥分开，其法向 $d$ 就是分离方向 (II)。

  \medskip
  \textbf{互斥很好看出：}若 (I) $g=\sum_i\lambda_i a_i$（$\lambda_i\ge0$）与 (II) $a_i^{\T} d\ge0\ \forall i$ 且 $g^{\T} d<0$ 同真，则 $g^{\T} d=\sum_i\lambda_i a_i^{\T} d\ge0$，与 $g^{\T} d<0$ 矛盾。故二者不能同真。（穷尽性是 Farkas 的实质，依赖锥的闭性，见文末注记。）
\end{frame}
...
\begin{frame}{引理 12.4 —— 方向化为乘子}
  ...
  \medskip
  \textbf{为什么等式乘子可自由变号：}把每个等式 $\grad c_i^{\T} d=0$ 写成两条不等式 $\grad c_i^{\T} d\ge0$ 与 $(-\grad c_i)^{\T} d\ge0$。对这套全是 $\ge0$ 的不等式用 Farkas，得非负系数 $\mu_i^{+}$（配 $+\grad c_i$）与 $\mu_i^{-}$（配 $-\grad c_i$）。令 $\lambda_i:=\mu_i^{+}-\mu_i^{-}$，它可正可负——这就是等式乘子的自由符号。
\end{frame}
...
\begin{frame}{引理 12.4 证明（$\Rightarrow$）—— 向锥投影}
  ...
  \proofskipnote
  设 $\grad f\notin N$，令 $\hat s$ 为 $N$ 中离 $\grad f$ 最近的点。

\textbf{为什么投影给出这两条？}$N$ 是闭凸锥，$\hat{s}=\arg\min_{s\in N}\|s-\grad f\|^{2}$（$N$ 闭保证最近点存在）。

(i) 对任意 $s\in N$，凸性给 $\hat{s}+t(s-\hat{s})\in N$（$0\le t\le1$）。令 $\varphi(t)=\|\hat{s}+t(s-\hat{s})-\grad f\|^{2}$，它在 $t=0$ 最小，故 $\varphi'(0)=2(s-\hat{s})^{\T}(\hat{s}-\grad f)\ge0$，即 $(s-\hat{s})^{\T}(\hat{s}-\grad f)\ge0$。

(ii) $N$ 是锥，故 $2\hat{s}\in N$、$\tfrac{1}{2}\hat{s}\in N$。代入 (i)：取 $s=2\hat{s}$ 得 $\hat{s}^{\T}(\hat{s}-\grad f)\ge0$；取 $s=\tfrac{1}{2}\hat{s}$ 得 $\hat{s}^{\T}(\hat{s}-\grad f)\le0$。两者合起来 $\hat{s}^{\T}(\hat{s}-\grad f)=0$。

代回 (i)：对所有 $s\in N$，$s^{\T}(\hat{s}-\grad f)\ge0$。

\medskip
\textit{投影 = 最近点；残差 $\hat{s}-\grad f$ 与 $N$ 中每个方向夹角不小于 $90^\circ$（钝角刻画），就像把一个梯度步投回约束集。}

\medskip
\end{frame}

\begin{frame}{引理 12.4 证明（$\Rightarrow$）—— 向锥投影（续）}
  投影的最优性给出
...
\begin{frame}{揭示：临界锥 $F_2(\ls)$}
  ...
  \medskip
  \textbf{逐项看为何恰好是 $0$：}对 $i\in\Eset$ 和 $\lambda_i^{*}>0$ 的起作用 $i$，$F_2$ 的定义给 $\grad c_i^{\T} w=0$，那项为 $0$；对 $\lambda_i^{*}=0$ 的 $i$，系数本身为 $0$，那项也为 $0$。所以整个和恰是 $0$，而非 $\ge0$。
...
\begin{frame}{定理 12.5 —— 二阶必要条件}
  ...
  \]}
  \textbf{(1) 为什么沿这条路径 $\Lag(z_k,\ls)=f(z_k)$：}$\Lag(z_k,\ls)=f(z_k)-\sum_i\lambda_i^{*}c_i(z_k)$。用引理~12.3(ii) 造的路径有 $c_i(z_k)=t_k\grad c_i(\xs)^{\T} w$：对 $w\in F_2$ 的强起作用约束（$\lambda_i^{*}>0$）有 $\grad c_i^{\T} w=0$ 故 $c_i(z_k)=0$；不起作用约束 $\lambda_i^{*}=0$。于是 $\sum_i\lambda_i^{*}c_i(z_k)=0$，每项都消失。

  \medskip
  \textbf{(2) 对 $\Lag(\cdot,\ls)$ 在 $\xs$ 作二阶 Taylor：}一阶项 $\grad_x\Lag(\xs,\ls)^{\T}(z_k-\xs)$ 因 KKT 平稳性 $\grad_x\Lag(\xs,\ls)=0$ 而消失，只剩二阶项，得 $f(z_k)=f(\xs)+\half(t_k/\|w\|)^{2}\,w^{\T}\nabla_{xx}^{2}\Lag\,w+o(t_k^{2})$。若曲率 $<0$ 则 $f(z_k)<f(\xs)$，与局部最优矛盾。
...
\begin{frame}{揭示：乘子是影子价格}
  ...
  \end{itemize}
  例 12.4 里 $\lambda_{1}^{*}=3/4$：把约束 1 放松 $\epsilon=0.1$，最优值约改善
  $0.1\times3/4=0.075$；而 $\lambda^{*}=0$ 的约束放松了也没有收益。
  \TLtakeaway{乘子是约束的边际价值——它的\emph{影子价格}。这就像 loss 对预算的敏感度（目标对约束的一个“梯度”）。}
\end{frame}
