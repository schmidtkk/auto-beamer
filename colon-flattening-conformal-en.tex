...
\begin{frame}{The plan, and why it can't fail to have an answer}
  ...
  \begin{itemize}
    \item We will find each planar coordinate of the flattening as a \textbf{harmonic} function: the smoothest, least-stretched field with the boundary pinned.
    \item ``Harmonic'' will become a \textbf{linear system} $\Lmat x=b$ whose matrix is symmetric positive-definite once the boundary is fixed.
...
\begin{frame}{References \& figure credits}
  ...
  \end{thebibliography}
  \smallskip
  {\scriptsize Figure credits (all openly licensed, via Wikimedia Commons): Gaussian-curvature surfaces --- Nicoguaro, CC0; colon anatomy --- Mikael Häggström, public domain; conformal map --- Oleg Alexandrov, public domain. All other figures original (TikZ).}
\end{frame}
