...
\begin{frame}{定理 8.1 证明思路（概要）}
  \textbf{核心想法：} 若 $\gamma_{1},\gamma_{2}$ 在 $t_{0}$ 时刻相交于 $m_{0}$，且速度分别为 $v_{1},v_{2}$，就在短时间窗内构造"快捷路径"互换连接。\proofskipnote
...
\begin{frame}{定理 8.7 证明（紧支撑情形）}
  \textbf{假设 $\mu_{1}\in\Pac$，$\mu_{0},\mu_{1}$ 紧支撑，证明 $\mu_{t_{0}}\in\Pac$，$t_{0}\in(0,1)$。}\proofskipnote
...
\begin{frame}{定理 8.7 证明（紧支撑收尾）}
  \textbf{步骤三（绝对连续性）。}\proofskipnote 由 $N\subset T^{-1}(T(N))$：
  \[
...
\begin{frame}{定理 8.7 证明续（非紧支撑情形）}
  \textbf{步骤四（非紧推广）。}\proofskipnote 若 $\mu_{t_0}\notin\Pac$，取体积零但 $\mu_{t_0}$-正测度的集 $Z_{t_0}$；
