...
\begin{frame}{定理 8.7 证明（紧支撑情形）}
  ...
  \textbf{步骤一。} 由定理 8.5（其 Lipschitz 性由推论 8.2 缩短引理给出），存在 Lipschitz 映射 $T$ 解 Monge 问题 $\mu_{t_0}\to\mu_1$。

  \textbf{步骤二（零测集）。} 设 $N$ 为零体积集。$T$ 的 Lipschitz 常数为 $C_K/\min(t_0,1-t_0)$（定理 8.5）；Lipschitz 映射的面积公式给出 $\mathrm{vol}(T(N))\le T_{\mathrm{Lip}}^n\cdot\mathrm{vol}(N)=0$，
  故 $\mu_1[T(N)]=0$（$\mu_1\in\Pac$，绝对连续，见 §0）。
