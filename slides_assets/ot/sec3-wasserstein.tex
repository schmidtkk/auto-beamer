...
\begin{frame}{$\Wp{p}$ 是距离（二）：粘合引理的陈述}
  ...
  \textbf{构造（测度论语言）。} 令 $\pi^{(23)}=\int_{\X}\pi^{(23)}_{x_{2}}\dd\mu_{2}(x_{2})$ 为正则分解，定义
  \[
    \pi^{(123)}(\dd x_{1}\dd x_{2}\dd x_{3})
    = \pi^{(12)}(\dd x_{1}\dd x_{2})\cdot\pi^{(23)}_{x_{2}}(\dd x_{3}).
  \]
  给定 $X_{2}$，$X_{1}$ 与 $X_{3}$ 条件独立。
...
\begin{frame}{定理 6.9 的证明（二）：弱收敛加矩收敛蕴含 $\Wp{p}\to0$}
  ...
  设 $\pi_{k}$ 为 $(\mu_{k},\mu)$ 的最优耦合。由弱收敛和引理 4.4，$(\pi_{k})$ 胶着，取子列 $\pi_{k}\wkto\pi$。定理 5.20 保证 $\pi=(\Id,\Id)_{\#}\mu$（平凡耦合）。

  \textbf{分割估计。} 利用
  \[
    \bigl(d(x,y)^{p}-R^{p}\bigr)_{+}
    \;\le\; 2^{p}\bigl[d(x,x_{0})^{p}\mathbf{1}_{d(x,x_{0})\ge R/2}
    + d(x_{0},y)^{p}\mathbf{1}_{d(x_{0},y)\ge R/2}\bigr],
  \]
  将 $\Wp{p}(\mu_{k},\mu)^{p}$ 分解为截断项（弱收敛给出 $\to0$）和尾部项（$p$ 阶矩均匀可积给出 $\to0$）：
  \[
    \limsup_{k}\Wp{p}(\mu_{k},\mu)^{p} = 0. \quad\square
  \]
