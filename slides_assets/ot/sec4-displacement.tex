...
\begin{frame}{作用量泛函与 Lagrange 量（7.2）}
  ...
  设 $M$ 为 Riemann 流形。\TLterm{Lagrange 量} $L(x,v,t)$ 在切丛 $TM\times[0,1]$ 上，
  \TLterm{作用量}为
  \[
  ...
  \textbf{作用量最小化曲线：} $A(\gamma)\le A(\tilde{\gamma})$（同端点）的曲线 $\gamma$。

  \textbf{关联最优传输成本：} $c(x,y):=\inf_{\gamma_{0}=x,\,\gamma_{1}=y}A(\gamma)$。
...
\begin{frame}{位移插值的构造（二）：$\R^{n}$ 中的显式公式}
  \textbf{特殊情形：} $\X=\R^{n}$，$c(x,y)=|x-y|^{2}$，最优映射 $T$ 存在。
...
% ── 定理 7.21 证明思路 ────────────────────────────────────────────────────────
\begin{frame}{定理 7.21 证明思路（概要）}
  \textbf{目标：} 设 $(\mu_{t})$ 满足条件 (ii)，需构造随机最小化曲线 $\Pi$ 使 $\mu_{t}=(e_{t})_{\#}\Pi$。

  \textbf{关键观察。} 若 $(\gamma_{t_{1}},\gamma_{t_{2}})$、$(\gamma_{t_{2}},\gamma_{t_{3}})$ 各为最优耦合，
  且 $(\mu_{t_{k}})$ 满足等号关系 (ii)，则 $(\gamma_{t_{1}},\gamma_{t_{3}})$ 也是最优耦合：
  \[
    E\,c^{t_{1},t_{3}}(\gamma_{t_{1}},\gamma_{t_{3}})
  ...
  \begin{enumerate}
    ...
    \item 引入 $\mu_{1/2}$，粘合三时刻最优耦合。
    \item 归纳地在所有 $j/2^{k}$ 时刻定义随机离散路径，由可测选择定理延伸为连续曲线。
    \item 令 $k\to\infty$，极限测度 $\Pi$ 即为所求动力学最优耦合。
  \end{enumerate}

  \textbf{逆向：} (i) $\Rightarrow$ (ii) 由 $\Pi$ 的定义直接计算（利用 $\gamma$ 的最小化性质）。

  \TLtakeaway{证明核心：等号关系 (ii) $\Rightarrow$ 二进逼近 $\Rightarrow$ 动力学最优耦合 $\Pi$；逆向由最小化性质直接得出。}
\end{frame}
...
\begin{frame}{Benamou–Brenier 公式（前瞻）}
  \textbf{动力学视角（Benamou–Brenier，1999）。} 在 $\R^{n}$ 中，寻找速度场 $v$ 使
  \[
    \partial_{t}\mu_{t}+\nabla\cdot(v_{t}\mu_{t})=0
    \quad\text{（连续性方程）},
  \]
  同时最小化动能 $\mathcal{A}(\mu,v)=\int_{0}^{1}\!\int_{\R^{n}}|v_{t}|^{2}\dd\mu_{t}\dd t$。
  ...
  \textbf{联系：} 最优速度场 $v_{t}=\nabla\varphi_{t}$（Brenier 势能的梯度），
  位移插值路径 $\mu_{t}=((1-t)\Id+tT)_{\#}\mu_{0}$ 是最优流。

  \textbf{意义：} 为 $W_2$ 提供 PDE 表示；Benamou–Brenier 据此设计数值算法；Otto 解读为 $\Ppp{2}(\R^{n})$ 上的 Riemann 度量（"Otto 度量"）。
