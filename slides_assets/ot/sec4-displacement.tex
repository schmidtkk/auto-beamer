...
\begin{frame}{定理 7.21 证明思路（一）：三时刻引理}
  \textbf{目标。} 证明 (ii) $\Rightarrow$ (i)：由三时刻加法等式构造路径空间上的动力学最优耦合 $\Pi$。
  ...
  \deflab{补一个事实：限制性质。}
  同一条最小化曲线穿过中点时，它在每个子区间仍最小化；这就是命题 7.16(ii) 的 restriction/Markov property。
...
\begin{frame}{定理 7.21 证明思路（二）：二进构造与极限}
  \deflab{补一个事实：二进时刻。}
  二进时刻是 $j/2^{k}$；它们在 $[0,1]$ 中稠密，所以先控制这些时刻，再用连续性传到所有时刻。
