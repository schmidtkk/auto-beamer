...
\begin{frame}{引理 4.3 的证明：逼近 + 单调收敛}
  ...
  \begin{enumerate}\small
    \item 用 $c-h$ 代替 $c$，不妨设 $c\ge0$（下半连续非负函数）。
    \item \textbf{逼近}（用 §0 下半连续\emph{性质 (a)}，inf-卷积）：把 $c$ 写成有界连续函数的逐点递增极限 $c=\lim_{\ell} c_\ell$，$c_\ell\nearrow c$，$c_\ell\in C_b(\X\times\Y)$。
    \item \textbf{先固定 $\ell$，再对 $k$ 取极限。} 弱收敛定义给出 $\int c_\ell\dd\pi_k\to\int c_\ell\dd\pi$；又因 $c_\ell\le c$ 有 $\int c_\ell\dd\pi_k\le\int c\dd\pi_k$，对 $k$ 取下极限：
    \[
      \int c_\ell\dd\pi \;=\; \lim_{k}\int c_\ell\dd\pi_k \;\le\; \liminf_{k}\int c\dd\pi_k .
    \]
  \end{enumerate}
  \textbf{收尾：对 $\ell$ 取上确界}（单调收敛定理，$c_\ell\nearrow c$）：左端 $\sup_\ell\int c_\ell\dd\pi=\int c\dd\pi$，右端与 $\ell$ 无关，故
  \[
    \int c\dd\pi \;\le\; \liminf_{k}\int c\dd\pi_k. \quad\square
  \]
  \TLtakeaway{关键：lsc 给出连续递增逼近（§0 性质 a），再用 $c_\ell\le c$ 把不等式传到 $\liminf$。}
\end{frame}

% ── 定理 4.1 的证明（一）：紧性 ───────────────────────────────────────────────
\begin{frame}{定理 4.1 的证明（一）：紧性}
  \textbf{目标：} 在 $\coup{\mu}{\nu}$ 中找成本最小的耦合——即 §0 "紧集上 lsc 必达最小"在测度空间的实现（$\coup{\mu}{\nu}$ 为"紧集"，$I[\pi]=\int c\dd\pi$ 为"lsc 目标"）。

  \textbf{步骤一：$\coup{\mu}{\nu}$ 是弱紧集。}
  \begin{itemize}\small
    \item $\{\mu\}\subset\Pp(\X)$ 胶着（Polish 空间上单点集自动胶着），$\{\nu\}$ 同理。
    \item 引理 4.4 $\Rightarrow$ $\coup{\mu}{\nu}$ 在 $\Pp(\X\times\Y)$ 中胶着。
    \item Prokhorov 定理 $\Rightarrow$ $\coup{\mu}{\nu}$ 预紧。
    \item \emph{闭性：}边际约束 $\int\varphi\dd\pi=\int\varphi\dd\mu$（对一切 $\varphi\in C_b(\X)$）在弱极限下保持，故弱极限仍以 $\mu,\nu$ 为边际 $\Rightarrow$ $\coup{\mu}{\nu}$ 闭。
    \item 闭 $+$ 预紧 $\Rightarrow$ \textbf{弱紧}。
  \end{itemize}

  \textbf{步骤二：取极小化列并提取极限。}
  \begin{itemize}\small
    \item 取 $(\pi_k)\subset\coup{\mu}{\nu}$ 使 $I[\pi_k]\to\inf I$。
    \item 由弱紧性，存在子列（仍记为 $\pi_k$）使 $\pi_k\wkto\pi^\star\in\coup{\mu}{\nu}$。
  \end{itemize}

\end{frame}

% ── 定理 4.1 的证明（二）：取极限并用下半连续 ────────────────────────────────
\begin{frame}{定理 4.1 的证明（二）：取极限并用下半连续}
  \textbf{步骤三：引理 4.3 给出最优性。}
  \begin{itemize}\small
    \item 令 $h(x,y)=a(x)+b(y)$，则 $h\in L^1(\pi_k)\cap L^1(\pi^\star)$，
      且 $\int h\dd\pi_k = \int a\dd\mu+\int b\dd\nu = \int h\dd\pi^\star$（边际不变）。
    \item 引理 4.3 $\Rightarrow$ $I[\pi^\star]\le\liminf_k I[\pi_k]=\inf I$。
    \item 又 $\pi^\star\in\coup{\mu}{\nu}$ $\Rightarrow$ $I[\pi^\star]\ge\inf I$。
    \item 故 $I[\pi^\star]=\inf I$，即 $\pi^\star$ 最优。\hfill$\square$
  \end{itemize}

  \textbf{证明全景回顾：}
  \begin{itemize}\small
    \item 步骤一：利用 Prokhorov 定理 $+$ 引理 4.4，证明 $\coup{\mu}{\nu}$ 弱紧。
    \item 步骤二：从极小化列提取弱收敛子列，极限仍在 $\coup{\mu}{\nu}$ 中。
    \item 步骤三：引理 4.3 确保极限的成本不超过下确界，故极限最优。
  \end{itemize}

  \TLtakeaway{三步：紧集 $\to$ 极限存在；下半连续 $\to$ 极限最优。每步都用到 Polish 空间的拓扑结构。}
\end{frame}
...
\begin{frame}{定理 4.6 的证明（一）：构造竞争计划}
  \textbf{假设 $\tilde{\pi}$ 不是最优的。} 则存在计划 $\hat{\pi}$ 使得
  \[
    \pf{(\mathrm{proj}_{\X})}{\hat{\pi}}=\tilde{\mu},\quad
    \pf{(\mathrm{proj}_{\Y})}{\hat{\pi}}=\tilde{\nu},\quad
    \text{但 } \int c\dd\hat{\pi} < \int c\dd\tilde{\pi}.
  \]
  令 $Z = \pi'[\X\times\Y]>0$。构造竞争计划：
  \[
    \hat{\pi}_{\mathrm{new}} \;:=\; (\pi - \pi') + Z\hat{\pi}.
  \]
  \textbf{验证边际：} 由 $\pi'=Z\tilde{\pi}$ 及 $\hat{\pi}$ 的边际，$\hat{\pi}_{\mathrm{new}}$ 的边际等于 $\mu,\nu$。

  \textbf{验证成本更低：}
  \[
    \int c\dd\hat{\pi}_{\mathrm{new}}
    = \int c\dd\pi - Z\int c\dd\tilde{\pi} + Z\int c\dd\hat{\pi}
    < \int c\dd\pi.
  \]
  （利用 $C(\mu,\nu)<+\infty$ 保证有限成本运算合法。）
...
\begin{frame}{定理 4.6 的证明（二）：唯一性的传递}
  \textbf{唯一性的传递。} 若 $\pi$ 是 $(\mu,\nu)$ 之间的\emph{唯一}最优计划，则类似论证可知 $\tilde{\pi}$ 也是 $(\tilde{\mu},\tilde{\nu})$ 之间的唯一最优计划。

  \textbf{理由。} 若 $\tilde{\pi}$ 不唯一，设 $\hat{\pi}\ne\tilde{\pi}$ 也是 $(\tilde{\mu},\tilde{\nu})$ 的最优计划。
  用同样的"替换"构造 $\hat{\pi}_{\mathrm{new}} = (\pi-\pi') + Z\hat{\pi}$，
  其成本与 $\pi$ 相同（因 $\hat{\pi}$ 与 $\tilde{\pi}$ 成本相同），
  但 $\hat{\pi}_{\mathrm{new}}\ne\pi$——与 $\pi$ 的唯一性矛盾。\hfill$\square$
  ...
  \TLtakeaway{证明关键：用子计划的"更好替代"拼出整体更优的计划；唯一性类似传递。条件化保持最优性是直接推论。}
\end{frame}

% ── 例 4.7 的原则解释 ──────────────────────────────────────────────────────────
\begin{frame}{例 4.7：条件化原则的威力}
  \TLwarnblock{为什么这个原则"简单但有力"}{
...
\begin{frame}{第三件好事：最优成本的凸性（定理 4.8）}
  ...
  $\theta\mapsto\nu_\theta\in\Pp(\Y)$ 可测，成本 $c\ge a+b$ 如定理 4.1。则
  \[
    C\!\left(\int_{\Theta}\!\mu_\theta\dd\lambda(\theta),\,
            \int_{\Theta}\!\nu_\theta\dd\lambda(\theta)\right)
    \;\le\;
  ...
  \textbf{证明思路。}
  \begin{itemize}\small
    \item 对 $\lambda$-a.e.\ $\theta$，定理 4.1 给出最优计划 $\pi_\theta\in\coup{\mu_\theta}{\nu_\theta}$。
    \item 令 $\pi:=\int_{\Theta}\pi_\theta\dd\lambda(\theta)$，其边际为 $\mu:=\int\mu_\theta\dd\lambda$，$\nu:=\int\nu_\theta\dd\lambda$，故 $\pi\in\coup{\mu}{\nu}$。
    \item 由 $C(\mu,\nu)\le I[\pi]$ 及 Fubini 定理：
      $C(\mu,\nu)\le \int_{\Theta} C(\mu_\theta,\nu_\theta)\dd\lambda(\theta)$。\hfill$\square$
  \end{itemize}

  \TLtakeaway{最优传输成本关于边际测度\emph{凸}：混合测度的传输成本不超过分别传输成本的平均。}
\end{frame}
