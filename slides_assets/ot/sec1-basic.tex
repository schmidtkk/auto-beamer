...
\begin{frame}{引理 4.3 的证明（一）：为什么可令 $c\ge0$}
  \textbf{先解释归约。} 因 $c\ge h$，函数 $c-h$ 非负；若已知引理对非负成本 $c-h$ 成立，则
  \[
    \begin{aligned}
      \int c\dd\pi
...
\begin{frame}{引理 4.3 的证明：逼近 + 单调收敛}
  \deflab{补一个事实：单调收敛定理。}
  若 $0\le c_{\ell}\nearrow c$，则 $\sup_{\ell}\int c_{\ell}\dd\pi=\lim_{\ell}\int c_{\ell}\dd\pi=\int c\dd\pi$。
...
\begin{frame}{定理 4.6 的证明（一）：构造竞争计划}
  \textbf{反证。} 令 $Z=\pi'[\X\times\Y]>0$，$\tilde{\pi}=\pi'/Z$。若 $\tilde{\pi}$ 不是最优的，
...
\begin{frame}{定理 4.8 的证明（一）：混合计划有正确边际}
  \textbf{选取局部最优计划。} 对 $\lambda$-a.e.\ $\theta$，定理 4.1 给出
