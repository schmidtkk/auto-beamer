...
\begin{frame}{引理 4.3：成本泛函的下半连续性}
  ...
  \begin{enumerate}\small
    ...
    \item 写 $c$ 为有界连续函数列的\emph{逐点递增极限}：$c=\lim_{\ell\to\infty} c_\ell$，$c_\ell\nearrow c$，$c_\ell\in C_b(\X\times\Y)$。
    \item 对每个固定 $\ell$：$\pi_k\wkto\pi \Rightarrow \int c_\ell\dd\pi_k\to\int c_\ell\dd\pi$（弱收敛定义）。
    \item 由单调收敛定理：
    \[
      \int c\dd\pi = \lim_{\ell}\int c_\ell\dd\pi
      = \lim_{\ell}\lim_{k}\int c_\ell\dd\pi_k
      \le \liminf_{k}\int c\dd\pi_k. \quad\square
    \]
  \end{enumerate}

  \TLtakeaway{下半连续成本泛函对弱收敛也下半连续——这让"取极限不增加成本"成为可能。}
\end{frame}
...
\begin{frame}{定理 4.1 的证明（一）：紧性}
  ...
  \begin{itemize}\small
    ...
    \item 边际条件在弱极限下保持 $\Rightarrow$ $\coup{\mu}{\nu}$ 闭。
  ...
  步骤二保证了极限的存在性与可行性。接下来用引理 4.3 证明最优性。
\end{frame}
