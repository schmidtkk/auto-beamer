...
\begin{frame}{定理 5.10 证明思路（一b）：验证 $\Gamma\subset\partial_c\psi$}
  \textbf{承接前页：$\psi$ c-凸，$\psi(x_0)=0$。目标：对每个 $(x,y)\in\Gamma$ 证 $\ctr{\psi}(y)=\psi(x)+c(x,y)$。}

  \begin{itemize}\small
    \item \textbf{上界：} 由竞争性不等式（c-变换定义），$\ctr{\psi}(y)=\inf_{x'}[\psi(x')+c(x',y)]\le\psi(x)+c(x,y)$。
    \item \textbf{下界：} 对任意 $x'\in\X$，考虑以 $(x,y)$ 为最后一步的链
      $(x_1,y_1),\ldots,(x_m,y_m)=(x,y)$。对 c-循环单调性的链不等式（在 $(x,y)$ 之后插入 $(x',y)$ 并绕回），可得
      \[
        \psi(x') \;\ge\; \psi(x) + c(x,y) - c(x',y).
      \]
      （即：从 $x'$ 出发把货送到 $y$，成本至少是"从 $x$ 到 $y$ 的成本"加上"放弃 $x'$ 所省的成本"。）\\
      整理：$\psi(x')+c(x',y)\ge\psi(x)+c(x,y)$ 对所有 $x'$ 成立，故
      $\ctr{\psi}(y)=\inf_{x'}[\psi(x')+c(x',y)]\ge\psi(x)+c(x,y)$。
    \item \textbf{结论：} 上下界相等，$\ctr{\psi}(y)=\psi(x)+c(x,y)$，即 $(x,y)\in\partial_c\psi$，所以 $\Gamma\subset\partial_c\psi$。
  \end{itemize}

  \TLtakeaway{c-循环单调性让每个 $(x,y)\in\Gamma$ 恰好"满足等号"：$\ctr{\psi}(y)-\psi(x)=c(x,y)$，即 $(x,y)$ 在 c-次微分中。}
\end{frame}
