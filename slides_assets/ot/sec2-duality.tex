...
\begin{frame}{命题 5.8 的证明（一）：先证 $\phi^{ccc}\le\phi^c$}
  \deflab{展开定义。}
  固定 $x\in\X$。\proofskipnote 把三次 c-变换完全展开，得到
  \[
    \phi^{ccc}(x)
...
\begin{frame}{命题 5.8 的证明（二）：再证 $\phi^{ccc}\ge\phi^c$}
  ...
  \]
  结合上一帧的反向不等式，得到 $\phi^{ccc}=\phi^c$。\proofskipnote
...
\begin{frame}{命题 5.8 的证明（三）：代回 c-凸性}
  ...
  \]
  反之，若 $\cctr{\psi}=\psi$，则 $\psi=(\ctr{\psi})^c$，它本身就是一个 c-变换，故 $\psi$ c-凸。\proofskipnote
...
\begin{frame}{定理 5.10 证明思路（一a）：定义势函数并验证 c-凸性}
  \textbf{核心构造（Rüschendorf/Rockafellar）。}\proofskipnote
...
\begin{frame}{定理 5.10 证明思路（一b）：上界与下界目标}
  \textbf{承接前页：$\psi$ c-凸，$\psi(x_0)=0$。}\proofskipnote \textbf{目标：对每个 $(x,y)\in\Gamma$ 证 $\ctr{\psi}(y)=\psi(x)+c(x,y)$。}
...
\begin{frame}{定理 5.10 证明思路（一c）：链下界的实际推导}
  \textbf{固定} $(x,y)\in\Gamma$。\proofskipnote 下面证明上一帧的下界目标。
...
\begin{frame}{定理 5.10 证明思路（二a）：积分恒等式}
  \textbf{承接前页的构造。}\proofskipnote 已有 $\psi$ c-凸、$\phi=\ctr{\psi}$，且 $\pi$ 集中在 $\partial_c\psi$。
...
\begin{frame}{定理 5.10 证明思路（二b）：弱对偶夹逼}
  令 $\mathcal A_c$ 表示满足可积条件的 c-凸候选集合。\proofskipnote 上一帧已给出一个候选 $\psi$，
  其对偶值等于 $\int c\dd\pi$。
...
\begin{frame}{定理 5.10 证明思路（二c）：逼近与可测性缺口}
  \deflab{本帧暂缓的技术点。}
  只暂缓把链式构造得到的 $\psi,\ctr{\psi}$ 选成可测版本：Villani 用推论 5.22 的可测选择思想配合 Lusin/Egorov，把它们改成几乎处处相同的可测函数。\proofskipnote 积分锁定本身不依赖这些选择细节。
...
\begin{frame}{定理 5.30 的证明（一）：同一个势函数控制所有最优计划}
  ...
  若这个集合只有一个元素，就说 c-次微分在 $x$ 处单值。\proofskipnote
...
\begin{frame}{定理 5.30 的证明（二）：从单值截面定义 $T$}
  \deflab{使用条件 (ii)。}
  令 $Z=\{x:\#\partial_c\psi(x)>1\}$，则 $\mu[Z]=0$。\proofskipnote
...
\begin{frame}{定理 5.30 的证明（三）：图耦合与唯一性}
  ...
  \]
  因为同一个 $\psi$ 给出同一个 $T$，每个最优耦合都等于这一个图耦合；唯一性与 Monge 可解性同时得到。\proofskipnote
