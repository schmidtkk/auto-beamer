...
\begin{frame}{方案，以及它为什么不可能没有解}
  ...
  \begin{itemize}
    \item 我们把摊平的每个平面坐标找成一个\textbf{调和}函数：边界钉住后，最光滑、拉伸最小的场。
    \item 「调和」会变成一个\textbf{线性系统} $\Lmat x=b$，边界一旦固定，它的矩阵就是对称正定。
...
\begin{frame}{参考文献 (References) 与图片出处}
  ...
  \end{thebibliography}
  \smallskip
  {\scriptsize 图片出处（均为开放授权，经 Wikimedia Commons）：Gauss 曲率示意 --- Nicoguaro，CC0；结肠解剖 --- Mikael Häggström，公有领域；共形映射 --- Oleg Alexandrov，公有领域。其余插图均为原创（TikZ）。}
\end{frame}
