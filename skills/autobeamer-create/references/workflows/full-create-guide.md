...
| Aspect | `academic-presentation` default | `passive-study` override |
|--------|---------------------------------|--------------------------|
...
- **Paraphrase, don't quote** — translate dense mathematical prose into accessible explanations (see `CLAUDE.md` passive-study style).
...
- A slide with only ≤ 3 short text-only bullets is too sparse — merge or enrich
- **Pure text-only bullet slides ≤ 20%** of total deck (stricter than `academic-presentation`)
